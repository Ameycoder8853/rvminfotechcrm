    ...
    <div className="space-y-6 animate-fade-in pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2 px-4 py-2 bg-[var(--surface)] border border-[var(--border)] rounded-xl shadow-sm">
          ...
          <span className="text-xs font-bold text-[var(--foreground-secondary)] uppercase tracking-wider">Representative Portal</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Planner Agenda list */}
        <div className="lg:col-span-2 bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 shadow-sm card-hover animate-scale-in">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-[10px] font-bold text-[var(--accent)] uppercase tracking-widest cursor-pointer hover:underline">Add Entry</span>
          </div>
          <div className="space-y-3">
            {(stats?.myTasks || []).map((task: any) => (
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-[var(--surface-hover)] border border-[var(--border)] rounded-lg text-[var(--accent)]">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-bold text-[var(--foreground-muted)] uppercase tracking-widest mt-0.5">
                      {task.startTime && task.endTime ? `${task.startTime} - ${task.endTime}` : "All Day"} | Type: <span className="text-[var(--foreground-secondary)]">{task.type}</span>
                    </p>
                  </div>
                </div>
                <span className={cn(
                  "inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider",
                  task.isCompleted ? "bg-[var(--success-muted)] text-[var(--success)]" : "bg-[var(--warning-muted)] text-[var(--warning)]"
                )}>
            ...
            {(!stats?.myTasks || stats?.myTasks.length === 0) && (
              <div className="text-center py-12">
                <CalendarIcon className="w-10 h-10 text-[var(--foreground-muted)] mx-auto mb-3" />
                <p className="text-xs font-bold text-[var(--foreground-muted)] uppercase tracking-widest">No diary planner tasks listed today.</p>
              </div>
            )}
          </div>
        </div>
        ...
        {hasLeads && (
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 shadow-sm card-hover">
            <div className="flex items-center justify-between mb-6">
              ...
              <span className="text-[10px] font-bold text-[var(--accent)] uppercase tracking-widest cursor-pointer hover:underline">View All</span>
            </div>
            <div className="space-y-4">
              ...
              {(!stats?.recentLeads || stats?.recentLeads.length === 0) && (
                <p className="text-center py-10 text-xs font-bold text-[var(--foreground-muted)] uppercase tracking-widest">No active leads in your pipeline</p>
              )}
            </div>
          </div>
        )}
